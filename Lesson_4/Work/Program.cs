...
int FindLocalMin(int[] array)
{
    for (var i = 0; i < array.Length; i++)
    {
        if (CheckNearestNumber(array, i))
            return i;
    }
    return -1;
}

bool CheckNearestNumber(int[] array, int index)
{
    // index = 1
    // 0 1 2
    for (var j = -1; j <= 1; j += 2)
    {
        var localMin = array[index];
        var checkedIndex = index + j;

        if (IsValidIndex(checkedIndex, array.Length))
        {
            if (localMin > array[checkedIndex])
                return false;
        }
    }
...
var localIndex = FindLocalMin(array);
